...
<head>
	...
	<title>大太阳建筑猎头--猎头服务管理--已过期的套餐</title>
...
<s:include value="left.jsp"/>
   ...
   <div class="content-right">
    	<div class="administrator-title"><strong>已过期的套餐</strong></div>
	     <s:form id="pageFrom" name="pageFrom" method="post" action="headHuntServiceManageAction!searchExpiredService.action">
	        <div class="inside-search">
	        	<ul>
	            	<li><strong>套餐名称:</strong>
	            		<s:textfield id="serviceNameHid" name="boughtSearchDto.serviceName" size="10" maxLength="20" cssStyle="display:none"/>
	            		<input type="text" id="serviceName" size="11" maxlength="20" value="<s:text name="boughtSearchDto.serviceName"/>"/>
	            	</li>
	                <li>购买日期:
	                	<s:textfield  id="fromOrderDateHid" name="boughtSearchDto.fromOrder"  size="7" cssStyle="display:none"/>
	                	<input type="text"  class="Wdate2" id="fromOrderDate" value="<s:text name="boughtSearchDto.fromOrder"/>" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'toOrderDate\')}'})" size="10" style="height:15px;"/>
	                		至
	                	<s:textfield  id="toOrderDateHid" name="boughtSearchDto.toOrder"  cssStyle="display:none"/>
	                	<input type="text" class="Wdate2" id="toOrderDate" value="<s:text name="boughtSearchDto.toOrder"/>" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'fromOrderDate\')}'})" size="10" style="height:15px;"/>
	                </li>
	                <li><a href="#" onclick="
	        ...
	        <div class="inside-tab">
	   	    <table width="100%" border="0" cellspacing="0" cellpadding="0">
	              <tr>
	                ...
	                <td width="10%" align="left" nowrap="nowrap"><strong>服务状态&nbsp;<strong></td>
	                <td width="9%" align="left" nowrap="nowrap"><strong>支付状态&nbsp;<strong></td>
	                <td width="10%" align="left"><strong>支付金额&nbsp;<strong></td>
	              ...
	              <s:if test="boughtList!=null">
		              ...
		              <tr>
		                ...
		                <td align="left" width="14%" height="40" align="left" title="<s:text name="serviceName"/>">
		                ...
		                <td align="left" width="9%"><s:text name="statusName"/></td>
		                <td align="left" width="10%">
		                ...
		                <td align="left" width="14%" title="<s:date name="orderDate" format="yyyy-MM-dd HH:mm"/>"><s:date name="orderDate" format="yyyy-MM-dd"/>&nbsp; </td>
		                <td align="left" width="14%" title="<s:date name="startDate" format="yyyy-MM-dd HH:mm"/>"><s:date name="startDate" format="yyyy-MM-dd"/>&nbsp; </td>
		                <td align="left" width="14%" title="<s:date name="endDate" format="yyyy-MM-dd HH:mm"/>"><s:date name="endDate" format="yyyy-MM-dd"/>&nbsp; </td>
		                <td align="left" width="10%" nowrap="nowrap">
	                		...
	                		<s:if test="serviceStatus == 2 || serviceStatus == 3">
	        ...
	        <div class="qx" >
	       	...
	       	<div class="shanchu" onclick="submitForm4ChangeAction(pageFrom,'selectAlls','headHuntServiceManageAction!remove.action')" title="删除">删除</div>
			<div class="num">
			   <s:property value="pageInfo.htmlPagingInfo" escapeHtml="false"/>
			</div>
			<s:hidden id="extension_id" name="extensionOrderId"/>
			<s:hidden id="extension_resId" name="extensionResId"/>
			<s:hidden name="decideExtensionFlag" value="2"/>
			<s:hidden name="buyDetailDto.serviceCode" id="view_id"/>
			<s:hidden name="buyDetailDto.serviceItem" id="view_itemid"/>
...
function init(){
	...
	$("#service_expired a").addClass("active");
}
</script>
</body>
</html>
