...
<div class="content-right">
    	<div class="administrator-title"><strong>已结束的职位</strong></div>
    	<s:form  id="pageFrom" name="pageFrom" method="POST" action="enterpriseCenterAction!getCompleteJobs.action">
        ...
        <div class="inside-tab">
   	    <table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                ...
                <td width="7%" align="left" class="inside-links"><a href="<s:url action="personalCenterAction!lookJobInfoDetail" namespace="/personalCenter">
		...
		<ul>
		  ...
		  <li class="botton3"><a href="javascript:void(0)" title="重新发布" onclick="republishJobs('myPublishedJobs');return false;">重新发布</a></li>
		</ul>
		</div>
      	<div class="num">
...
function changeMyPublishingJobs(checkboxName){
	if(checkStatus(checkboxName)==false) {
		...
		if(window.confirm("你确认要进行此操作吗？")){
			document.pageFrom.action='enterpriseCenterAction!deleteMyPublishedJob.action?requestType=4';
...
function republishJobs(checkboxName){
	if(checkStatus(checkboxName)==false) {
		alert('请至少选中一个职位');
	}else{
		if(window.confirm("你确认要进行此操作吗？")){
			document.pageFrom.action='enterpriseCenterAction!republishJobs.action';
			document.pageFrom.method='post';
			document.pageFrom.submit();
		}
	}
...
function init(){
	...
	$("#position_ended a").addClass("active");
}
