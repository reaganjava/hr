...
<head>
	...
	<title>大太阳建筑猎头--系统管理--已结束的证书需求列表</title>
...
<body onload="init()">
	...
	<div class="content">
		...
		<div class="content-right">
			<div class="administrator-title"><strong>已结束的证书需求</strong></div>
			<s:form id="searchForm" name="searchForm" method="post" action="certNeedsManageAction!viewExpiredCertNeedsList.action">
				<div class="inside-search">
					<ul>
						...
						<li>需求状态:
							<s:select name="certNeedsMap.certNeedsStatus" list="#{null:'所有','0':'未猎证','1':'已猎证','2':'暂停猎证' }" listKey="key" listValue="value"></s:select>
						</li>
						<li>
			...
			<div class="inside-tab">
				<s:form id="listForm" name="listForm" method="post">
					<input type="hidden" name="from" value="ExpiredCertNeeds" />
					...
					<div id="question2" style="display:none; background-color:#ffffff;">
						<div class="mid-border">
							<h5><span class="close">X</span>重新发布证书需求</h5>
							<ul>
								<li>发布时间：
									<input type="text" id="effDate" name="certNeedsMap.effDate" size="20" class="Wdate"
									readonly="readonly"
									value="<s:property value='currentDate'/>" 
									onclick="WdatePicker({isShowClear:false,dateFmt:'yyyy-MM-dd',errDealMode:1,minDate:'%y-%M-{%d}',maxDate:'#F{$dp.$D(\'expDate\')}'})"/>
								</li>
								<li>有效期至：
									<input type="text" id="expDate" name="certNeedsMap.expDate" size="20" class="Wdate"
									readonly="readonly"
									value="<s:property value='extendationDate'/>" 
									onclick="WdatePicker({isShowClear:false,dateFmt:'yyyy-MM-dd',errDealMode:1,minDate:'#F{$dp.$D(\'effDate\')}'})"/>
								</li>
								<li><a href="#"><img onclick="republishCertNeeds()" src="<%=path%>/images/cxfb.png" width="81" height="28" alt="延长时间"/></a></li>
							</ul>
						</div>
					</div>
				</s:form>
			</div>
			<s:form  id="pageFrom" name="pageFrom" method="POST" action="certNeedsManageAction!viewExpiredCertNeedsList.action">
					<s:hidden name="certNeedsMap.certTypeName"></s:hidden>
					<s:hidden name="certNeedsMap.certName"></s:hidden>
					<s:hidden name="certNeedsMap.specialFieldName"></s:hidden>
					<s:hidden name="certNeedsMap.certNeedsStatus"></s:hidden>
				...
				<div class="system-botton3">
					<ul>
						...
						<li class="botton3">
							<a href="javascript:void(0)" class="openDiv" id="Div1" rel="question2" title="重新发布">
								重新发布
							</a>
						</li>
					</ul>
	...
	<script type="text/javascript">
		...
		function republishCertNeeds(){
			document.listForm.action = "certNeedsManageAction!republishCertNeeds.action";
			document.listForm.submit();
		}
	</script>
	<script type="text/javascript">
		...
		function init(){
			...
			$("#position_expiredCertNeeds").addClass("active");
		}
 	</script>
</body>
</html>
