        ...
        defaultSetting: function (caller) { // NOT GENERALLY USED, NEEDED FOR THE API, DO NOT TOUCH
            ...
            for (i = 0; i < rules.length; i++) {
                ...
                case "required":
                    ...
                    break;
                case "requiredYZM":
                	_requiredYZM(caller, rules);
                	break;
                case "requiredPassword":
                	_requiredPassword(caller, rules);
                	break;
                case "custom":
                ...
                case "confirm":
                    ...
                    break;
                case "notEqual":
                    _notEqual(caller, rules, i);
                    break;
                case "funcCall":
                	_funcCall(caller, rules, i);
                	break;
                case "passwordRegister":
                	_passwordRegister(caller, rules, i);
                	break;
                case "checkNullRelatedS":
                	_checkNullRelatedS(caller, rules, i);
                	break;
                case "isCheckImg":
                	_isCheckImg(caller, rules, i);
                	break;
                case "isCheckVedio":
                	_isCheckVedio(caller, rules, i);
                	break;
                case "clearPromp":
                	_clearPromp(caller, rules, i);
                	break;
            ...
            function _required(caller, rules) { // VALIDATE BLANK FIELD
                ...
                if (callerType == "text" || callerType == "password" || callerType == "textarea") {
                    if (!$.trim($(caller).val())) {// 必需字段，去掉两边空格
            ...
            function _requiredYZM(caller, rules){
                if ($(caller).val() == null || $(caller).val() == "") {
                    $.validationEngine.isError = true;
                    promptText += $.validationEngine.settings.allrules["requiredYZM"].alertText + "<br />";
                }
            }
            function _requiredPassword(caller, rules){
                if ($(caller).val() == null || $(caller).val() == "") {
                    $.validationEngine.isError = true;
                    promptText += $.validationEngine.settings.allrules["requiredPassword"].alertText + "<br />";
                }
            }
			...
			/**检查关联项**/
			function _checkNullRelatedS(caller, rules, position) {
                var str1 = eval(rules[position + 1]);
                var relatedId = eval(rules[position + 2]);
                var relatedId3 = eval(rules[position + 3]);

                if(Ltrim($(caller).attr('value')) == "" && Ltrim($('#' + relatedId).attr('value')) != ""){
                	promptText += $.validationEngine.settings.allrules["checkNullRelatedS"].alertText1 +"," + str1 + $.validationEngine.settings.allrules["checkNullRelatedS"].alertText2;
                	$.validationEngine.isError = true;
                }
                if($(caller).attr('value') == "" && relatedId3 != undefined){
                	//$.validationEngine.closePrompt($("#"+relatedId3));
            	    deleteItself = "." + $("#"+relatedId3).attr("id") + "formError";
		            if ($(deleteItself)[0]) {
		                $(deleteItself).stop();
		                $(deleteItself).remove();
		            }
			...
			/**根据不为空清楚其它控件上的提示**/
			function _clearPromp(caller, rules, position) {
                var relatedId1 = eval(rules[position + 1]);
                var relatedId2 = eval(rules[position + 2]);
                if(Ltrim($(caller).attr('value')) == ""){
                	if(relatedId1 != "" && relatedId1 != undefined){
	            	    deleteItself = "." + $("#"+relatedId1).attr("id") + "formError";
			            if ($(deleteItself)[0]) {
			                $(deleteItself).stop();
			                $(deleteItself).remove();
			            }
			            //alert($('#' + relatedId1).attr('value'));
			            document.getElementById(relatedId1).value="";
			            try{
				            document.getElementById(relatedId1).select();
		            ...
		            if(relatedId2 != "" && relatedId2 != undefined){
	            	    deleteItself = "." + $("#"+relatedId2).attr("id") + "formError";
			            if ($(deleteItself)[0]) {
			                $(deleteItself).stop();
			                $(deleteItself).remove();
			            }
			            document.getElementById(relatedId2).value="";
			            try{
			            	document.getElementById(relatedId2).select();
			            	document.execCommand('Delete');
			            }catch(e){
			            	//alert(e);
			            }
			...
			/**检查图片**/
			function _isCheckImg(caller, rules, position) {

				if(Ltrim($(caller).attr('value')) == "") return;
				var reg = /^(.+\.(JPEG|jpeg|JPG|jpg|GIF|gif|BMP|bmp|PNG|png))$/;
                if(!reg.test($(caller).attr('value'))){
                	promptText += $.validationEngine.settings.allrules["isCheckImg"].alertText;
                	$.validationEngine.isError = true;
                }

			}
			/**检查视频**/
			function _isCheckVedio(caller, rules, position) {

				if(Ltrim($(caller).attr('value')) == "") return;
				var reg = /^(.+\.(asf|ASF|wma|WMA|wmv|WMV|avi|AVI|wav|WAV|mpeg|MPEG|rm|RM|FLV|flv))$/;
                if(!reg.test($(caller).attr('value'))){
                	promptText += $.validationEngine.settings.allrules["isCheckVedio"].alertText;
                	$.validationEngine.isError = true;
                }
            ...
            function _notEqual(caller, rules, position) { // VALIDATE FIELD MATCH
                confirmField = rules[position + 1];

                if ($(caller).attr('value') == $("#" + confirmField).attr('value')) {
                    $.validationEngine.isError = true;
                    promptText += $.validationEngine.settings.allrules["notEqual"].alertText + "<br />";
                }
            }
            ...
            function _passwordRegister(caller, rules, position) { // VALIDATE PASSWORD
                startLength = eval(rules[position + 1]);
                endLength = eval(rules[position + 2]);
                feildLength = $(caller).attr('value').length;
                if (feildLength < startLength || feildLength > endLength) {
                    $.validationEngine.isError = true;
                    promptText += $.validationEngine.settings.allrules["passwordRegister"].alertText + startLength + $.validationEngine.settings.allrules["passwordRegister"].alertText2 + endLength + $.validationEngine.settings.allrules["passwordRegister"].alertText3 + "<br />";
                }
            }
            ...
            //验证多个邮件
            function _emails(caller, rules, position) {
                var min = eval(rules[position + 1]);
                var max = eval(rules[position + 2]);
                var value;
                try {
                    ...
                    if(!reg.test(value)){
                        promptText += $.validationEngine.settings.allrules["emails"].alertText;
                        $.validationEngine.isError = true;
                    }
                } catch (e) {
                    promptText += $.validationEngine.settings.allrules["emails"].alertText;
